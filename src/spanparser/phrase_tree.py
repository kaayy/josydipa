...
class PhraseTree(object):
    ...
    def compare_dis(self, gold):
        mybracks = self.brackets(True)
        goldbracks = gold.brackets(True)

        mybracks = set(x for x in mybracks if x[0].startswith("DIS:"))
        goldbracks = set(x for x in goldbracks if x[0].startswith("DIS:"))

        correct = 0
        for gb in goldbracks:
            if gb in mybracks:
                correct += 1
        return FScore(correct, len(mybracks), len(goldbracks))
