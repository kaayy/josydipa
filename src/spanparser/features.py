...
class FeatureMapper(object):
    ...
    @staticmethod
    def vocab_init(trees, verbose=True):
        """
        ...
        for i, tree in enumerate(trees):
            ...
            if verbose:
                print('\rTree {}'.format(i), end='', file=sys.stderr)
                sys.stdout.flush()
        ...
        if verbose:
            print('Loading features from {} trees'.format(len(trees)))
    ...
    def __init__(self, trees, verbose=True):
    ...
    def l_action(self, index):
        if index == 0:
            ...
            return 'label-' + self.ldict.keys()[index - 2]
    ...
    def gold_data_from_trees(self, trees):
        result = []
        for tree in trees:
